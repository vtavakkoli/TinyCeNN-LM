...
def make_replacement(original: nn.Module, cfg: LayaLabConfig) -> BaseLayaReplacementAttention:
    if cfg.architecture == "integrated_memory_v22":
        return IntegratedMemoryV22Attention(
            original, cfg.feature_dim, cfg.local_kernel
        )
    if cfg.architecture == "memory_fusion":
        return MemoryFusionAttention(
            original, cfg.feature_dim, cfg.memory_rank, cfg.local_kernel
        )
...
def replacement_parameters(model: nn.Module) -> int:
    return sum(
        ...
        if isinstance(m, BaseLayaReplacementAttention)
...
def choose_candidate_layers(
    ...
    """Choose central layers, optionally prioritizing one ModernBERT attention type.

    Integrated-memory conversion should attack full-attention layers first.
    Sliding-attention layers already have sub-quadratic locality and, more
    importantly, require preserving a per-query sliding mask that a global
    kernel-memory approximation intentionally does not model.
    """
    ...
    if preferred_attention_type is not None:
        indices.sort(
            key=lambda i: (
                0
                if str(getattr(model.encoder.layers[i], "attention_type", ""))
    ...
    indices.sort(
        key=lambda i: (
            abs(i - center),
            0
            if getattr(model.encoder.layers[i], "attention_type", "")
            == "sliding_attention"
            else 1,
        )
    )
