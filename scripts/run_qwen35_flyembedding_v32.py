...
def parse_args():
    p = argparse.ArgumentParser(description="FlyEmbedding-v3.2 preservation-first experiment")
    ...
    p.add_argument("--gate-groups", type=int, default=32)
    p.add_argument("--gate-hidden", type=int, default=64)
    ...
    p.add_argument("--output-dir", default="results/flyembedding_v32_qwen35_08b")
...
def train(student, teacher, train_batches, val_batches, updates, device, args):
    trainable = freeze_qwen_train_fly_v32(student)
    core = student.fly_embedding_v32_core
    opt = torch.optim.AdamW([
        {"params":[core.down.weight, core.up.weight, core.gate_down.weight, core.gate_down.bias, core.gate_up.weight, core.gate_up.bias], "lr":args.lr_core},
    ...
    for step in range(1, updates+1):
        ...
        if step == 1 or step % 25 == 0 or step == updates:
            print(
                ...
                f"scale={row['residual_scale']:.6f} gate={row['gate_mean']:.4f}±{row['gate_std']:.4f} "
                f"ups={step/max(time.perf_counter()-t0,1e-9):.2f}",
                flush=True
            )
...
def main():
    ...
    print("Loading Qwen + FlyEmbedding-v3.2 student...", flush=True)
    ...
    identity = assert_fly_embedding_v32_identity(
    ...
    report = {
        "architecture":"Qwen3.5-0.8B + FlyEmbedding-v3.2 preservation-first residual adapter",
        ...
        "gate_design":{
            "type":"token-dependent grouped gate",
            "gate_groups":args.gate_groups,
            "gate_hidden":args.gate_hidden,
        },
