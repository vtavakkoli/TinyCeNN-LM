...
from tinycenn_lm.smollm2_memory_fusion import (
    ...
    save_smollm2_memory_fusion,
    structural_assertions,
)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Progressive distillation of SmolLM2 into full Memory Fusion attention")
    p.add_argument("--base-model", default=DEFAULT_SMOLLM2)
    p.add_argument("--output-dir", default="checkpoints/smollm2-memory-fusion-r48")
    ...
    p.add_argument("--memory-rank", type=int, choices=(48, 64), default=48)
    p.add_argument("--group-size", type=int, default=5)
    p.add_argument("--calibration-steps", type=int, default=30)
    p.add_argument("--calibration-lr", type=float, default=2e-4)
    p.add_argument("--final-max-tokens", type=int, default=350_000)
    p.add_argument("--final-grad-accum", type=int, default=4)
...
def representation_loss(student_hidden, teacher_hidden) -> torch.Tensor:
    max_idx = len(student_hidden) - 1
    candidates = (6, 12, 18, 24, 30)
    indices = tuple(i for i in candidates if i <= max_idx)
    terms = []
    for idx in indices:
...
def main() -> None:
    ...
    print("\n=== Phase 1: teacher-attention calibration ===")
    for stage_id, group in enumerate(groups, start=1):
        replace_attention_layers(student, config, group)
        trainable = freeze_for_group_calibration(
            student, group, train_output_projection=config.train_output_projection
        )
        ...
        for step in range(1, args.calibration_steps + 1):
            if (time.perf_counter() - start) / 60.0 >= args.max_runtime_minutes * 0.60:
                print("calibration time budget reached; continuing to remaining groups")
                break
            ...
            if not torch.isfinite(loss):
                raise RuntimeError("non-finite layerwise Memory Fusion calibration loss")
            ...
            if step == 1 or step % args.log_every == 0 or step == args.calibration_steps:
                print(
                    f"rank={args.memory_rank} stage={stage_id}/{len(groups)} layers={group[0]}-{group[-1]} "
                    f"step={step}/{args.calibration_steps} align={last_loss:.4f} "
                    f"nmse={last_nmse:.4f} cosine_dist={last_cos:.4f}"
                )
    ...
    # If the calibration time slice ended early, still create every replacement.
    replace_attention_layers(student, config, range(num_layers))
    structural_assertions(student)

    print("\n=== Phase 2: end-to-end CE + KL + hidden-state distillation ===")
    trainable = freeze_for_global_training(
        student, train_output_projection=config.train_output_projection
    )
    params = parameter_summary(student)
    ...
    while final_tokens < args.final_max_tokens:
        ...
        with amp():
            ...
            loss = args.ce_weight * s_out.loss + args.kl_weight * kl + args.hidden_weight * hidden
            scaled = loss / args.final_grad_accum
        if not torch.isfinite(scaled):
            raise RuntimeError("non-finite global Memory Fusion distillation loss")
        ...
        if updates == 1 or updates % args.log_every == 0:
            print(
                f"rank={args.memory_rank} global update={updates} tokens={final_tokens:,} "
                f"ce={last['ce']:.4f} kl={last['kl']:.4f} hidden={last['hidden']:.4f} grad={grad_norm:.3f}"
            )
    ...
    (output_dir / "smollm2_memory_fusion_training_report.json").write_text(
