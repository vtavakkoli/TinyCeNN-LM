...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Sequential FunctionGemma Memory Fusion conversion. V1 replaces only "
            "the model's original full-attention anchor layers and leaves Gemma3 "
            "sliding-window attention unchanged."
        )
    )
    p.add_argument("--base-model", default=DEFAULT_FUNCTIONGEMMA)
    p.add_argument("--model-revision", default="main")
    p.add_argument("--output-dir", default="checkpoints/functiongemma-memory-fusion-sequential-r64")
    ...
    p.add_argument("--target-layers", default="auto", help="auto or comma-separated full-attention layer indices")
...
def chunked_distillation_kl(student_logits, teacher_logits, temperature: float, chunk: int) -> torch.Tensor:
    """KL with small token chunks to cap FunctionGemma's large-vocabulary temporaries."""
...
def train_one_replacement(*, teacher, student, layer_idx, batch_iter, probe_blocks,
    ...
    for step in range(1, args.max_layer_steps + 1):
        ...
        if should_check:
            ...
            candidate = {
                "step": step, "nmse": nmse, "cosine": cosine,
            ...
            print(
                ...
                f"=> {'PASS' if passed else 'continue'}", flush=True
            )
            if passed:
                best = candidate
                break
...
def load_progress(output_dir: Path, student, config, target_layers, *, resume: bool):
    ...
    if accepted:
        ...
        if missing:
            raise RuntimeError(f"resume checkpoint missing Memory Fusion keys: {missing[:8]}")
...
def load_in_progress(output_dir: Path, student, config, target_layers, accepted_layers, *, resume: bool):
    ...
    incompatible = student.load_state_dict(payload["attention_state"], strict=False)
    prefixes = tuple(f"model.layers.{i}.self_attn." for i in list(accepted_layers) + [expected_current])
    missing = [k for k in incompatible.missing_keys if prefixes and k.startswith(prefixes)]
    if missing:
        raise RuntimeError(f"in-progress checkpoint missing Memory Fusion keys: {missing[:8]}")
    print(f"RESUME CURRENT LAYER: layer={expected_current}, rounds_completed={payload.get('rounds_completed', 0)}", flush=True)
    return payload
...
def main() -> int:
    ...
    teacher = AutoModelForCausalLM.from_pretrained(
        args.base_model, revision=args.model_revision, dtype=dtype, attn_implementation="sdpa"
    ...
    text_config = student.config.get_text_config(decoder=True)
    if text_config.model_type != "gemma3_text":
        raise ValueError(f"expected gemma3_text, got {text_config.model_type}")
    target_layers = target_layers_for(student, args.target_layers)
    print(f"Gemma3 layers={text_config.num_hidden_layers}; full-attention targets={target_layers}", flush=True)

    config = Gemma3MemoryFusionConfig(feature_dim=args.feature_dim, memory_rank=args.memory_rank)
    ...
    for layer_idx in target_layers:
        ...
        if (time.perf_counter() - start) / 60 >= args.max_runtime_minutes * 0.70:
            atomic_json(output_dir / "sequential_run_status.json", {
                "status": "paused_runtime_budget", "accepted_layers": accepted_layers,
                "next_layer": layer_idx, "message": "Rerun with --resume to continue."
            })
            return 0

        print("\n" + "=" * 100, flush=True)
        print(f"FUNCTIONGEMMA FULL-ATTENTION REPLACEMENT: layer {layer_idx}", flush=True)
        ...
        if in_progress is not None and int(in_progress["current_layer"]) == layer_idx:
            ...
            print(f"before replacement NLL={pre_probe_nll:.6f}; Δteacher={pre_probe_nll-teacher_probe_nll:+.6f}", flush=True)
        ...
        for local_round in range(1, max_rounds + 1):
            ...
            report = train_one_replacement(
                teacher=teacher, student=student, layer_idx=layer_idx,
                batch_iter=batch_iter, probe_blocks=probe_blocks,
                teacher_probe_nll=teacher_probe_nll, pre_probe_nll=pre_probe_nll,
                args=round_args, device=device, amp=amp,
            )
        ...
        save_progress(
            output_dir, student, config, target_layers, accepted_layers, layer_reports,
            stage=f"accepted_layer_{layer_idx}"
        )
        remove_in_progress(output_dir)
        in_progress = None
        print(f"✅ accepted FunctionGemma full-attention layer {layer_idx}", flush=True)
    ...
    atomic_torch(output_dir / "functiongemma_memory_fusion_full_state.pt", {
    ...
    atomic_json(output_dir / "sequential_run_status.json", {
        "status": "complete", "accepted_layers": accepted_layers, "target_layers": target_layers
    })
...
if __name__ == "__main__":
    try:
        ...
        print("Interrupted. Saved accepted/current-layer checkpoints remain resumable.", file=sys.stderr, flush=True)
        raise
